...
# 可排序的映射项类型别名：字段值类型不限，故以 Any 表达，使排序键可返回任意可比较值。
SortableItem = Mapping[str, Any]
...
def apply_sort(items: list[SortableItem], sorts: list[SortField]) -> list[SortableItem]:
    """对映射列表做多键稳定排序。
    ...
    for spec in reversed(sorts):
        # 排序键按字段取值；异构字段统一以 Any 视作可比较，缺失键按 None 排序。
        result.sort(
            key=lambda item, f=spec.field: cast(Any, item.get(f)),
