...
* 当前阶段：阶段 B 进行中；`B-001`～`B-008` 已完成，下一任务 `B-009`。
* 整体完成度：`32 / 326` 个原子开发任务完成（约 `9.8%`）。
* 当前分支：`main`，HEAD 为 `47353ab`（B-007 提交）。
...
* Task 编号：`B-009`
* Task 名称：发布项目列表 API 契约
* 当前状态：待开始。
* 前置依赖：`B-008`（已完成）。
* 当前目标：发布 OpenAPI 列表接口。
* 验收标准：筛选、分页和最大页大小契约通过。
...
`TASKS.md` 共有 326 个原子任务，已完成 32 个（`A-001`～`A-024`、`B-001`～`B-008`），剩余 294 个。
