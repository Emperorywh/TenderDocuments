...
* 当前阶段：阶段 C 进行中；`C-001`～`C-015` 已完成，下一任务 `C-016`。
* 整体完成度：`58 / 326` 个原子开发任务完成（约 `17.8%`）。
* 当前分支：`main`，HEAD 为 `287ef4f`（C-014 提交）。
...
* Task 编号：`C-016`
* Task 名称：建立 DocumentRelation 迁移
* 当前状态：待开始。
* 前置依赖：`C-015`（已完成）。
* 当前目标：建立替代、补充和引用关系表。
* 验收标准：自引用和非法跨项目关系被约束拒绝。
...
`TASKS.md` 共有 326 个原子任务，已完成 58 个（`A-001`～`A-024`、`B-001`～`B-019`、`C-001`～`C-015`），剩余 268 个。
