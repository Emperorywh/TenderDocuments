...
* 当前阶段：阶段 C 进行中；`C-001`～`C-013` 已完成，下一任务 `C-014`。
* 整体完成度：`56 / 326` 个原子开发任务完成（约 `17.2%`）。
* 当前分支：`main`，HEAD 为 `19e1d95`（C-012 提交）。
...
* Task 编号：`C-014`
* Task 名称：建立 Document 迁移
* 当前状态：待开始。
* 前置依赖：`B-001`、`A-016`（已完成）。
* 当前目标：建立逻辑文件表。
* 验收标准：Document 必须归属于一个 project_id。
...
`TASKS.md` 共有 326 个原子任务，已完成 56 个（`A-001`～`A-024`、`B-001`～`B-019`、`C-001`～`C-013`），剩余 270 个。
