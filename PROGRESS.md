...
* 当前阶段：阶段 D 进行中；`C-001`～`C-034` 全部完成，`D-001`～`D-018` 已完成，下一任务 `D-019`。
* 整体完成度：`94 / 326` 个原子开发任务完成（约 `28.8%`）。
* 当前分支：`main`，HEAD 为 `D-018` 提交（待创建）。
...
* 当前状态：待开始（`D-018` 已完成）。
* 前置依赖：`D-006`（已完成）。
* 当前目标：可重试与不可重试错误目录；文件损坏和网络错误进入不同分支。
* 阻塞风险：错误分类为纯领域规则（错误码 → 可重试/不可重试），可在当前环境完整验证。
...
* 实现摘要：新增 analysis application `RecoveryAction`（SCHEDULED_RETRY/FAILED）与 `RecoveryOutcome`；infrastructure `stuck_task_recovery.py`（`recover_stuck_task`：标记当前 RUNNING 尝试 FAILED（finished_at、error_code=HEARTBEAT_TIMEOUT，保留不覆盖），按已用重试=总尝试-1 决定任务 RUNNING→RETRY_SCHEDULED（可重试，待 D-020 重投递）或 →FAILED（超 max_retries）；守卫 RUNNING 幂等；不在内部提交）。
* 验证结果（2026-07-23）：7 项测试通过——**卡死恢复标记尝试 FAILED 且任务 RETRY_SCHEDULED**、超 max_retries 转 FAILED、**恢复后重投递重新领取生成新尝试、旧失败记录保留不覆盖（SPEC 11.2）**、非 RUNNING 返回 None、重复恢复幂等、未知任务 NotFoundError、自定义错误码；全量 468 项通过；ruff、pyright 0 错误。
* 说明：恢复经 RUNNING→RETRY_SCHEDULED（状态机合法）；RETRY_SCHEDULED→DISPATCHED 重投递属 D-020（SPEC.md 第 11.2、12.2 节）。

### D-017 实现卡死任务扫描

* 实现摘要：新增 analysis application `StuckTask`（卡死任务识别结果：含稳定归属、attempt_number、last_activity 判定依据）与 infrastructure `stuck_task_scan.py`（`find_stuck_tasks(session, *, now, heartbeat_timeout_seconds)`：以 COALESCE(heartbeat_at, started_at) < now-timeout 识别卡死的 RUNNING 任务+尝试，回退 started_at 覆盖领取后未刷新即崩溃；扫描只读不改状态，timeout 由配置注入）。
* 验证结果（2026-07-23）：7 项测试通过——**过期心跳被识别**、**正常心跳不误判**、无心跳回退 started_at 过期识别、无心跳新近不误判、已完成尝试（SUCCEEDED）旧活动不识别、返回含稳定归属与 last_activity、无任务返回空；全量 461 项通过；ruff、pyright 0 错误。
* 说明：扫描只读，恢复（重新领取或失败转换）属 D-018（SPEC.md 第 12.2 节）。

### D-016 实现任务心跳

* 实现摘要：迁移 0012 为 task_attempts 增加 heartbeat_at（DateTime 可空，与 started_at 分离）；TaskAttemptModel 增 heartbeat_at 列；新增 infrastructure `task_heartbeat.py`（`refresh_heartbeat(session, task_id, *, now)`：取任务最新 RUNNING 尝试刷新 heartbeat_at，返回 True；无 RUNNING 尝试返回 False；不在内部提交）。
* 验证结果（2026-07-23）：6 项测试通过——heartbeat_at 列存在且可空、刷新 RUNNING 尝试心跳为 now、无 RUNNING 尝试返回 False、**周期多次刷新心跳随时间前进**、多次尝试中刷新作用于最新 RUNNING（旧完成尝试不动）、未知任务返回 False；D-005 迁移回归通过；全量 454 项通过；ruff、pyright 0 错误。
* 说明：心跳写在执行尝试上（每次执行一条 RUNNING 尝试），卡死任务扫描（D-017）据此识别过期心跳（SPEC.md 第 12.2 节）。

### D-015 实现任务幂等结果提交

* 实现摘要：新增 analysis infrastructure `task_result.py`（`submit_task_result`：守卫式 UPDATE 仅 RUNNING→SUCCEEDED/FAILED，rowcount=0 表示已提交返回 False 幂等空操作；成功/失败分别标记当前 RUNNING attempt 完成——finished_at、status、error_code（失败记录稳定错误分类））；任务不存在抛 NotFoundError；不在内部提交。可重试失败（RUNNING→RETRY_SCHEDULED）属 D-019/D-020。
* 验证结果（2026-07-23）：11 项测试通过——成功提交任务 SUCCEEDED 且尝试完成、失败提交记录错误码、**重复提交幂等空操作（第二次 False，任务/尝试不重复）**、非 RUNNING（PENDING/DISPATCHED/SUCCEEDED/FAILED/CANCELLED/RETRY_SCHEDULED）返回 False 状态不变、未知任务 NotFoundError、**并发提交只一个成功（任务只转换一次）**；全量 448 项通过；ruff、pyright 0 错误。
* 说明：守卫式 UPDATE ... WHERE status='RUNNING' 保证“只提交一次”，重复消息不产生重复正式结果（SPEC.md 第 11.3 节），与 D-013 领取一致的原子模式。
...
`TASKS.md` 共有 326 个原子任务，已完成 94 个（`A-001`～`A-024`、`B-001`～`B-019`、`C-001`～`C-034`、`D-001`～`D-018`），剩余 232 个。
