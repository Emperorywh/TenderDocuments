...
* 当前阶段：阶段 A——工程与架构基线；`A-001`～`A-021` 已完成，正在向 `A-022` 推进。
* 整体完成度：`21 / 326` 个原子开发任务完成（约 `6.4%`）。
* 当前分支：`main`，HEAD 为 `c7cf0e5`（A-020 提交）。
...
* Task 名称：实现状态转换通用验证器
* 当前状态：待开始。
* 前置依赖：`A-021`、`A-012`（已完成）。
* 当前目标：实现显式状态转换校验入口。
* 验收标准：合法转换通过，非法转换返回稳定错误码。
...
`TASKS.md` 共有 326 个原子任务，已完成 21 个（`A-001`～`A-021`），剩余 305 个。
