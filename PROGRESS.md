...
* 当前阶段：阶段 C 进行中；`C-001`～`C-019` 已完成，下一任务 `C-020`。
* 整体完成度：`62 / 326` 个原子开发任务完成（约 `19.0%`）。
* 当前分支：`main`，HEAD 为 `8209155`（C-018 提交）。
...
* Task 编号：`C-020`
* Task 名称：定义 FileSecurityScanner 端口
* 当前状态：待开始。
* 前置依赖：`A-005`、`C-019`（已完成）。
* 当前目标：建立文件安全扫描端口。
* 验收标准：领域层不依赖具体扫描器。
...
`TASKS.md` 共有 326 个原子任务，已完成 62 个（`A-001`～`A-024`、`B-001`～`B-019`、`C-001`～`C-019`），剩余 264 个。
