...
* 当前阶段：阶段 A——工程与架构基线；`A-001`～`A-022` 已完成，正在向 `A-023` 推进。
* 整体完成度：`22 / 326` 个原子开发任务完成（约 `6.7%`）。
* 当前分支：`main`，HEAD 为 `37f86ad`（A-021 提交）。
...
* Task 编号：`A-023`
* Task 名称：建立模块 README 模板
* 当前状态：待开始。
* 前置依赖：`A-005`（已完成）。
* 当前目标：建立职责、入口、状态、依赖和禁区模板。
* 验收标准：未参与实现者可据模板说明示例模块边界。
...
`TASKS.md` 共有 326 个原子任务，已完成 22 个（`A-001`～`A-022`），剩余 304 个。
